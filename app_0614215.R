...
midcastServer <- function(id, data.ts, mdl, psi.mle) {
  moduleServer(id, function(input, output, session) {
    observeEvent(input$run_midcast, {
      req(data.ts(), mdl(), psi.mle())
      # Imputation only works if there are NAs
      na.idx <- which(is.na(data.ts()))
      if (length(na.idx) == 0) {
        output$midcast_message <- renderText("No missing values in selected series—nothing to impute.")
        output$midcast_plot <- renderPlot({ plot(1, type='n', axes=FALSE, xlab="", ylab="") })
        return()
      }
      
      # ---- Midcast extraction code ----
      # Use your earlier logic: sigex.adhocextract, reg.effect, etc.
      ...
      output$midcast_plot <- renderPlot({
        ...
        ggplot(df, aes(x = Year)) +
          geom_ribbon(aes(ymin = Lower, ymax = Upper), fill = "#d7263d", alpha = 0.35) +
